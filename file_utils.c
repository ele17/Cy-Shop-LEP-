...
void writeProductToFile(Produit produit) {
  ...
  if (file != NULL) {
    fprintf(file, "%s %d %d %.2f %d %s\n", produit.nom, produit.reference,
            produit.quantite, produit.prix, produit.taille, produit.datePeremption);
    fclose(file);
  }
}

Produit readProductFromFile(char *nomProduit) {
  Produit produit = {"", 0, 0, 0.0, 0, ""};
...
Produit readProductByRefFromFile(int referenceProduit) {
    ...
    if (file != NULL) {
        while (fscanf(file, "%s %d %d %f %d %s", produit.nom, &produit.reference,
                      &produit.quantite, &produit.prix, &produit.taille, produit.datePeremption) != EOF) {
            if (produit.reference == referenceProduit) {
                fclose(file);
                return produit;
            }
        }
    ...
    // Si le produit n'est pas trouvé, retourne un produit avec une référence de -1
    produit.reference = -1;
    return produit;
}
...
void updateProductInFile(Produit produit) {
  ...
  if (originalFile != NULL && tempFile != NULL) {
    while (fscanf(originalFile, "%s %d %d %f %d %s\n", produitTemp.nom,
                  &produitTemp.reference, &produitTemp.quantite,
                  &produitTemp.prix, &produitTemp.taille, produitTemp.datePeremption) != EOF) {
      ...
      if (strcmp(produitTemp.nom, produit.nom) == 0) {
        fprintf(tempFile, "%s %d %d %.2f %d %s\n", produit.nom,
                produit.reference, produit.quantite, produit.prix,
                produit.taille, produit.datePeremption);
      } else {
               // fichier temporaire
        fprintf(tempFile, "%s %d %d %.2f %d %s\n", produitTemp.nom,
                produitTemp.reference, produitTemp.quantite, produitTemp.prix,
                produitTemp.taille, produitTemp.datePeremption);
      }
    }
    fclose(originalFile);
    fclose(tempFile);
    ...
    *numberOfProducts = 0;
    ...
    if (file != NULL) {
        ...
        while (fscanf(file, "%s %d %d %f %d %s\n", produitTemp.nom, &produitTemp.reference, 
                      &produitTemp.quantite, &produitTemp.prix, &produitTemp.taille, produitTemp.datePeremption) != EOF) {
...
void ajouterAchat(Client *client, Produit produit, int quantite) {
    ...
    newAchat.prixUnitaire = produit.prix;  // Ajoutez cette ligne

    // Copiez le nom du produit dans l'objet Achat
